...
def add_chore(file_name):
    ...
    chore_time = input("Enter the approx. time it will take to complete this chore: ")
...
def mark_chore(file_name):
    print("Mark chore")
    view_chores(file_name)
    chore_title = input("Enter the chore title that you want to mark as complete (Write it exactly as shown): ")
    chore_day = input("Enter the day you completed this chore: ")
    chore_instructions = input("Enter any notes for this chore (if you would like to leave as blank, enter 'none'): ")
    chore_time = input("Enter the time it took you to complete this chore: ")
    chore_lists = []
    with open(file_name, "r") as chore_file:
        reader = csv.reader(chore_file)
        for row in reader:
